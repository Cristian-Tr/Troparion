...
function genereazaEnciclopedia() {
    ...
    config.forEach(item => {
        ...
        item.date.forEach(subiect => {
            ...
            dt.innerHTML = `<h4>${subiect.subjectName}</h4>`;
            
            const dd = document.createElement('dd');
            dd.innerHTML = `<h4>${subiect[item.camp] || "Informație lipsă"}</h4>`;
...
function activeazaMeniuri() {
    document.querySelectorAll('.column-page ul').forEach(buton => {
        buton.onclick = function() {
            const listaMea = this.parentElement.querySelector('li');
            if(!listaMea) return;
            
            document.querySelectorAll('.column-page li').forEach(li => {
                if (li !== listaMea && window.getComputedStyle(li).display === 'block') {
                    slideToggleCustom(li, 200);
                }
            });
